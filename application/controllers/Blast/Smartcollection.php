...
class Smartcollection extends CI_Controller
{
    ...
    public function blast()
    {
        ...
        //     $limit = 500000;
        // }
        $data_channel = $this->data_lead->live_query("select channel,count(id) as jumlah_data FROM data_lead WHERE send_status IS NULL AND DATE(blast_date) = '$tgl_now'")->result();
        if (count($data_channel) > 0) {
            foreach ($data_channel as $ch) {
                ...
                $data_blast = $this->data_lead->live_query("select * FROM data_lead WHERE send_status IS NULL AND channel='$ch->channel'")->result();
                $this->data_lead->live_query("UPDATE data_lead  SET send_status=1 WHERE send_status IS NULL AND channel='$ch->channel'")->result();
                $setting = false;
    ...
    public function email_blast($data_blast, $sid, $period_sch, $ch, $setting)
    {
        ...
        foreach ($data_blast as $db) {
            ...
            // $nama            = $data_additional->NAMA;
            // $todo            = $data_additional->TODO;
            $email_customer    = $db->email;
            $tagihan        = $db->tagihan;
            // $duedate        = $data_additional->DUEDATE;
            $tgl_input        = date('Y-m-d H:i:s');
            // $cprod            = $data_additional->CPROD;
            $periode        = $period_sch;
            $id_template    = $id_message;
            // $group_id        = $data_additional->GROUP_ID;
            // $regional        = $data_additional->REGIONAL;
