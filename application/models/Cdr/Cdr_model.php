...
{
   public function create_cdr(string $call_id, string $starttime, string $transtime, string $endtime, string $queue, string $exten, string $outbound_cid, 
   string $dst, string $duration, string $billsec, string $a_status, string $b_status, string $pbx_domain, string $recordingfile)
   {
      ...
      $insert = $this->db->insert('cdr', $data);
      if ($insert) {
         $response = array();
